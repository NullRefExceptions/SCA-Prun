...
{
  debug(0) << "lo =" << lo << " high=" << hi << "\n";
  assert(hi < indices.size() && lo < indices.size());
  if (indices.size() == 1)
    return indices[0];
  uint64_t mid = lo + (hi - lo) / 2;
  if (indices[mid] <= val && (indices.size() - 1 == mid || indices[mid + 1] > val))
    return indices[mid];
  else if (indices[mid] > val)
    return binarySearchIndices(indices, lo, mid > 0 ? mid - 1 : 0, val);
  else
    return binarySearchIndices(indices, mid + 1, hi, val);
}
