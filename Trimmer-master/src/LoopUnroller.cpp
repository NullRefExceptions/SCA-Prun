...
/*This file contain methods for loop unrolling such as getting trip count,
whether the loop is a File I/O loop, whether the loop should be unrolled etc
...
bool LoopUnroller::checkUnrollHint(BasicBlock *hdr, LoopInfo &LI, Module *module)
{
  ...
  {
    /*
    如果循环展开时，llvm无法给出展开的次数上线，那么会形成剩余loop，因此需要删除标记以确保
    不会发生重复展开
     */
    firstInst->setMetadata("context_loop", nullptr);
    return true;
  }
  return false;
}

bool LoopUnroller::shouldSimplifyLoop(BasicBlock *BB, LoopInfo &LI, Module *m)
{
  if (!checkUnrollHint(BB, LI, m))
    return false;
...
bool LoopUnroller::doUnroll(TargetLibraryInfo *TLI, AssumptionCache &AC, unsigned tripCount)
{
  ...
  ulo.Force = true;
  ulo.Runtime = allowRuntime;
  ulo.UnrollRemainder = false;

  TargetTransformInfo ttfi(module->getDataLayout());
  int UnrollResult = (int)UnrollLoop(L, ulo, LI, &SE, &DT, &AC, &ttfi, &ORE, PreserveLCSSA); // unrollRemaineder set to false currently
  if (!UnrollResult)
  {
